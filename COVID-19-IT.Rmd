...
mod4 = nls(y ~ richards(x, th1, th2, th3), data = data, start = start,
           ...
           control = nls.control(maxiter = 1000, tol = 1))
...
peak = c(which(peaks(df$grad2)), which(peaks(df$grad3)), which(peaks(df$grad4)))
...
pred4 = cbind(df, "fit" = predict(mod4, newdata = df), "lwr" = NA, "upr" = NA)
pred4[df$x > max(data$x), c("lwr", "upr")] = predictMBB.nls(mod4, df[df$x > max(data$x),])
...
ylim = c(0, max(pred2$upr, pred3$upr, pred4$upr, na.rm=TRUE))
```
...
ggplot(df, aes(x = date, y = y)) + 
  ...
  geom_vline(data = df[peak,], aes(xintercept = date), 
             lty = 2, col = cols[2]) +
  coord_cartesian(ylim = range(df$grad2)*c(0,10)) +
