...
$$
where $\theta_1$ is the upper horizontal asymptote, $\theta_2$ represents the x-value at the inflection point of the symmetric growth curve, and $\theta_3$ represents a scale parameter (and $1/theta_3$ is the growth-rate parameter that controls how quickly the curve converges transitions to the upper asymptote).
...
tab <- tableGrob(signif(tab,4),  theme = ttheme)
ggsave(grid.arrange(tab), 
       file = "COVID-19-IT_table.png", 
...
ggsave(file = "COVID-19-IT_graph.png", width = 7, height = 6, dpi = 300)
```
