...
data = data.frame(date = COVID19$data,
                  ...
                  w = COVID19$totale_casi/mean(COVID19$totale_casi))
...
mod1 = nls(y ~ exponential(x, th1, th2), weights = w, data = data, start = start)
summary(mod1)
```
...
```{R}
mod2 = nls(y ~ SSlogis(x, Asym, xmid, scal), weights = w, data = data)
summary(mod2)
```
...
```{R}
mod3 = nls(y ~ SSgompertz(x, Asym, b2, b3), weights = w, data = data,
...
mod4 = nls(y ~ richards(x, th1, th2, th3), weights = w, 
